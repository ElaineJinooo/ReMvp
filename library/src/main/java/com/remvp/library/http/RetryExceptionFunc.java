...
public class RetryExceptionFunc implements Function<Observable<? extends Throwable>, Observable<?>> {
    private static final String TAG = "RetryExceptionFunc";
    /* retry次数*/
    private int count = 0;
    /*延迟*/
                ...
                .zipWith(Observable.range(1, count + 1), new BiFunction<Throwable, Integer, Wrapper>() {
                    ...
                    public ObservableSource<?> apply(@NonNull Wrapper wrapper) throws Exception {
                        ...
                        if ((wrapper.throwable instanceof ConnectException
                                ...
                                || shouldRetry(errCode, wrapper))
                                && wrapper.index < count + 1) { //如果超出重试次数也抛出错误，否则默认是会进入onCompleted
                            Log.i(TAG, "apply: 发起重复请求");
     ...
     *
     * @param errCode 错误码
     * @param wrapper 错误异常和次数
     * @return
     */
    protected boolean shouldRetry(String errCode, Wrapper wrapper) {
        return false;
    }
