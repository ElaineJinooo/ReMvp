...
public class CustomToast {
    ...
    private static final int TOAST_TAG = 0x01;
    ...
    private CustomToast(Context context) {
        ...
        if (viewWithTag == null) {
            v = ((Activity) context).getLayoutInflater().inflate(
                    R.layout.toast_custom, container);
            v.setTag(TOAST_TAG);
        } else {
            v = viewWithTag;
        }
    ...
    public static CustomToast makeText(Context context, CharSequence message, int HIDE_DELAY) {
        if (result == null) {
            result = new CustomToast(context);
        } else {
    ...
    public static CustomToast makeText(Context context, int resId, int HIDE_DELAY) {
        String mes = "";
        try {
